...
/proc/get_region_accesses(code)
	switch(code)
		...
		if(2) //security
			return list(ACCESS_SEC_DOORS, ACCESS_WEAPONS, ACCESS_SECURITY, ACCESS_BRIG, ACCESS_ARMORY, ACCESS_FORENSICS_LOCKERS, ACCESS_COURT, ACCESS_HOS, ACCESS_LEGION, ACCESS_NCR, ACCESS_NCR_ARMORY, ACCESS_BOS, ACCESS_ENCLAVE, ACCESS_BOS_PALADIN, ACCESS_BOS_KNIGHT)
		if(3) //medbay
...
/proc/get_access_desc(A)
	switch(A)
		...
		if(ACCESS_BOS)
			return "BOS Access"
		if(ACCESS_BOS_PALADIN)
			return "BOS Paladin Access"
		if(ACCESS_BOS_KNIGHT)
			return "BOS Knight Access"
		if(ACCESS_LEGION)
